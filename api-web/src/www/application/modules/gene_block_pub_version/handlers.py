...
class List(handlers.standard.ListHandler):
    ...
    def create_query(self, data):
        query = GeneBlockPublicationVersion.objects
...
class Get(handlers.standard.GetHandler):
    def get_data(self, data):
        pub_version_helper = components.GeneBlockPublicationVersionHelper()
        gene_block_version = GeneBlockPublicationVersion.objects.get(pk=data['pub_id'])
        return {
            ...
            'publications':     pub_version_helper.load_publications(gene_block_version.id),
...
class Delete(handlers.standard.DeleteHandler):
    def delete(self, data):
        gene = GeneBlockPublicationVersion.objects.get(pk=data['pub_id'])
        gene.is_disabled = True
        gene.save()
        return 1
class Active(handlers.standard.DeleteHandler):
    def delete(self, data):
        block = GeneBlockPublicationVersion.objects.get(pk=data['id'])
        # Update all block to inactive
        GeneBlockPublicationVersion.objects.filter(gene_id=block.gene_id, gene_block_id=block.gene_block_id, version_status='active').update(version_status='inactive')
