...
class List(handlers.standard.ListHandler):
    ...
    def create_query(self, data):
        query = VariationBlockEffectVersion.objects
...
class Get(handlers.standard.GetHandler):
    def get_data(self, data):
        effect_version_helper = components.VariationBlockEffectVersionHelper()
        variation_block_version = VariationBlockEffectVersion.objects.get(pk=data['eff_id'])
        return {
            ...
            'risks':            effect_version_helper.load_risks(variation_block_version.id),
        }
...
class Delete(handlers.standard.DeleteHandler):
    def delete(self, data):
        variation = VariationBlockEffectVersion.objects.get(pk=data['eff_id'])
...
class Active(handlers.standard.DeleteHandler):
    def delete(self, data):
        block = VariationBlockEffectVersion.objects.get(pk=data['id'])
        # Update all block to inactive
        VariationBlockEffectVersion.objects.filter(variation_id=block.variation_id, variation_block_id=block.variation_block_id, version_status='active').update(version_status='inactive')
