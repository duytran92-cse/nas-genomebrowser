...
class List(handlers.standard.ListHandler):
    ...
    def create_query(self, data):
        query = VariationBlockVersion.objects
...
class Get(handlers.standard.GetHandler):
    def get_data(self, data):
        variation_block_version = VariationBlockVersion.objects.get(pk=data['text_id'])
        return {
            ...
            'text':             variation_block_version.text,
            'title':            variation_block_version.title,
...
class Delete(handlers.standard.DeleteHandler):
    def delete(self, data):
        block = VariationBlockVersion.objects.get(pk=data['id'])
...
class Active(handlers.standard.DeleteHandler):
    def delete(self, data):
        ...
        VariationBlockVersion.objects.filter(variation_id=block.variation_id, variation_block_id=block.variation_block_id, version_status='active').update(version_status='inactive')
