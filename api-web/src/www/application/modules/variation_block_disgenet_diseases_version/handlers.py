...
class List(handlers.standard.ListHandler):
    ...
    def create_query(self, data):
        query = VariationBlockDisgenetDiseasesVersion.objects
...
class Get(handlers.standard.GetHandler):
    def get_data(self, data):
        disgenet_diseases_version_helper = components.VariationBlockDisgenetDiseasesVersionHelper()
        variation_block_version = VariationBlockDisgenetDiseasesVersion.objects.get(pk=data['pub_id'])
        return {
            ...
            'disgenet_diseases':     disgenet_diseases_version_helper.load_disgenet_diseases(variation_block_version.id),
        }
...
class Delete(handlers.standard.DeleteHandler):
    def delete(self, data):
        variation = VariationBlockDisgenetDiseasesVersion.objects.get(pk=data['pub_id'])
...
class Active(handlers.standard.DeleteHandler):
    def delete(self, data):
        block = VariationBlockDisgenetDiseasesVersion.objects.get(pk=data['id'])
        # Update all block to inactive
        VariationBlockDisgenetDiseasesVersion.objects.filter(variation_id=block.variation_id, variation_block_id=block.variation_block_id, version_status='active').update(version_status='inactive')
