...
class List(handlers.standard.ListHandler):
    ...
    def create_query(self, data):
        query = VariationBlockPublicationTextVersion.objects
...
class Get(handlers.standard.GetHandler):
    def get_data(self, data):
        pub_text_version_helper = components.VariationBlockPublicationTextVersionHelper()
        variation_block_version = VariationBlockPublicationTextVersion.objects.get(pk=data['pub_text_id'])
        return {
            ...
            'publication_text':     pub_text_version_helper.load_publication_text(variation_block_version.id),
        }
...
class Delete(handlers.standard.DeleteHandler):
    def delete(self, data):
        variation = VariationBlockPublicationTextVersion.objects.get(pk=data['pub_text_id'])
...
class Active(handlers.standard.DeleteHandler):
    def delete(self, data):
        block = VariationBlockPublicationTextVersion.objects.get(pk=data['id'])
        # Update all block to inactive
        VariationBlockPublicationTextVersion.objects.filter(variation_id=block.variation_id, variation_block_id=block.variation_block_id, version_status='active').update(version_status='inactive')
