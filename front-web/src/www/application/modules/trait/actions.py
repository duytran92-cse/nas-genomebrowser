...
class List(common_actions.BaseAction):
    class ListRenderer(genopedia_renderers.trait.ListRenderer):
        pass
    ...
    def load_table_data(self, page_number=2, letter='a'):
        return components.TraitStore(self.get_container()).list(page_number=page_number, letter=letter)
    def GET(self):
...
class View(common_actions.BaseAction):
    def create_page_widget(self, page_id):
        ...
        page = components.TraitStore(self.get_container()).get(page_id['trait_name'])
        page = page['data']['record']
        for position in page['blocks']:
            for block in page['blocks'][position]:
                block['key'] = 'trait'
                page_widget.add_block(position, block)
        page_widget.add_params_page('trait')
        page_widget.add_params_entity(page_id['trait_name'])
        return page_widget
    def GET(self):
        page_context = self.create_page_context()
