...
class Update(actions.crud.UpdateAction, common_actions.BaseAction):
    def create_form(self):
        gene_block = components.DiseaseBlockStore(self.get_container()).get(self.params['block_id'])
    ...
    def load_form(self, form):
        result = components.DiseaseBlockStore(self.get_container()).get(self.params['block_id'])
        if result['status'] == 'ok':
            record = result['data']['record']
            form.set_things({
                'page': 'disease',
                'page_title': record['disease_title']
            })
    ...
    def process_form_data(self, data):
        ...
        res = components.DiseaseBlockStore(self.get_container()).update(data, self.params['block_id'])
        rs = components.DiseaseBlockStore(self.get_container()).helper(res['data']['pk'])
        self.params['page_title'] = rs['data']['record']['title']
        return res

    def handle_on_success(self, messages):
        return HttpResponseRedirect('/disease/%s' % (self.params["page_title"]))
