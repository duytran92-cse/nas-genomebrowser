...
class List(common_actions.BaseAction):
    class ListRenderer(genopedia_renderers.treatment.ListRenderer):
        pass
    ...
    def load_table_data(self, page_number=2, letter='a'):
        return components.TreatmentStore(self.get_container()).list(page_number=page_number, letter=letter)
    def GET(self):
...
class View(common_actions.BaseAction):
    def create_page_widget(self, page_id):
        ...
        page = components.TreatmentStore(self.get_container()).get(page_id['treatment_name'])
        page = page['data']['record']
        for position in page['blocks']:
            for block in page['blocks'][position]:
                block['key'] = 'treatment'
                page_widget.add_block(position, block)
        page_widget.add_params_page('treatment')
        page_widget.add_params_entity(page_id['treatment_name'])
        return page_widget
    def GET(self):
        page_context = self.create_page_context()
