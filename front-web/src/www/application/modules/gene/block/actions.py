...
class Update(actions.crud.UpdateAction, common_actions.BaseAction):
    def create_form(self):
        gene_block = components.GeneBlockStore(self.get_container()).get(self.params['block_id'])
    ...
    def load_form(self, form):
        result = components.GeneBlockStore(self.get_container()).get(self.params['block_id'])
        if result['status'] == 'ok':
            record = result['data']['record']
            form.set_things({
                'page': 'gene',
                'page_title': record['gene_title']
            })
    ...
    def process_form_data(self, data):
        ...
        res = components.GeneBlockStore(self.get_container()).update(data, self.params['block_id'])
        rs = components.GeneBlockStore(self.get_container()).helper(res['data']['pk'])
        self.params['page_title'] = rs['data']['record']['title']
        return res

    def handle_on_success(self, messages):
        return HttpResponseRedirect('/gene/%s' % (self.params["page_title"]))
