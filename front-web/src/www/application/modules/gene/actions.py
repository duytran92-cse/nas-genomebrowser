...
class List(common_actions.BaseAction):
    class ListRenderer(genopedia_renderers.gene.ListRenderer):
        pass
    def create_table(self):
        table = widgets.table.DataTable()
        table.renderer = self.ListRenderer()
        return table
    def load_table_data(self, page_number=2, letter='a', params= {}):
        return components.GeneStore(self.get_container()).list(params= params, page_number=page_number, letter=letter)
    def GET(self):
        ...
        data = self.load_table_data(page_number, letter, self.params)
...
class View(common_actions.BaseAction):
    def create_page_widget(self, page_id):
        ...
        page = components.GeneStore(self.get_container()).get(page_id['gene_name'])
        if page['status'] == 'ok':
            ...
            page_widget.add_params_page('gene')
            page_widget.add_params_entity(page_id['gene_name'])
        return page_widget
    def GET(self):
        page_context = self.create_page_context()
