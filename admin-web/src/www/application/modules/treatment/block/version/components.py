...
class TreatmentBlockVersionStore(object):
    ...
    def list(self, params={}, sortkey='id', sortdir='desc', page_number=1):
        ...
        data = self.container.call_api(settings.GENOME_BROWSER_API_URL + '/treatment_block_version/list', GET=params)
        return data['data']
    def get(self, text_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/treatment_block_version/get', GET={'text_id': text_id})
    def update(self, data, text_id):
        data['text_id'] = text_id
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/treatment_block_version/update', POST=data)
    def delete(self, text_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/treatment_block_version/delete', POST={'id': text_id})
    def active(self, text_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/treatment_block_version/active', POST={'id': text_id})
...
class FullPageContext(page_contexts.FullPageContext):
    def __init__(self, params, container):
        ...
        if 'treatment_id' in params and  'treatment_block_id' in params:
            self.subsubmenu.create_menu_group('list', 'Text Versions', '/treatment/detail/%s/block/detail/%s/version/list' % (str(params['treatment_id']), str(params['treatment_block_id'])), 'zmdi-border-all')
