...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/treatment/detail/%s/comment/update/%s">Edit</a>' % (row['page_id'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/treatment/detail/%s/comment/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['page_id'], row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Comments')
        table.set_subtitle('List of treatment comments')
        table.create_button('create', '/treatment/detail/%s/comment/create' % (self.params['treatment_id']), 'zmdi-plus')
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        table_form_data['idPage'] = 'treatment'
...
class Create(actions.crud.CreateAction):
    ...
    def process_form_data(self, data):
        data['entity'] = self.params['treatment_id']
        data['page'] = 'treatment'
...
class Delete(actions.crud.DeleteAction):
    def GET(self):
        ...
        return HttpResponseRedirect('/treatment/detail/%s/comment/list' % self.params['treatment_id'])
