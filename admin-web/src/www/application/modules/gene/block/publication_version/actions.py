...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/gene/detail/%s/block/detail/%s/pub_version/update/%s">Edit</a>' % (row['gene'], row['gene_block'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/gene/detail/%s/block/detail/%s/pub_version/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['gene'], row['gene_block'], row['id'])
            html += '    <a class="btn btn-xs btn-info" href="/gene/detail/%s/block/detail/%s/pub_version/active/%s">Active</a>'  % (row['gene'], row['gene_block'], row['id'])
            html += '</div>'
            return html
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        return components.GeneBlockPublicationVersionStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
...
class Update(actions.crud.UpdateAction):
    ...
    def create_form(self):
        ...
        form.set_title('Gene block publication versions')
    ...
    def load_form(self, form):
        result = components.GeneBlockPublicationVersionStore(self.get_container()).get(self.params['pub_id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.GeneBlockPublicationVersionStore(self.get_container()).update(data, self.params['pub_id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.GeneBlockPublicationVersionStore(self.get_container()).delete(self.params['pub_id'])
        return HttpResponseRedirect('/gene/detail/%s/block/detail/%s/pub_version/list' % (self.params['gene_id'], self.params['gene_block_id']))
