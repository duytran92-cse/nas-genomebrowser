...
class GeneBlockStore(object):
    ...
    def list(self, params={}, sortkey='id', sortdir='desc', page_number=1):
        ...
        data = self.container.call_api(settings.GENOME_BROWSER_API_URL + '/gene_block/list', GET=params)
        return data['data']
    def get(self, id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/gene_block/get', GET={'id': id})
    def create(self, data):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/gene_block/create', POST=data)
    def update(self, data, id):
        data['id'] = id
        data['verify'] = True
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/gene_block/update', POST=data)
    def delete(self, id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/gene_block/delete', POST={'id': id})
