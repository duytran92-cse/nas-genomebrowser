...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/gene/detail/%s/comment/update/%s">Edit</a>' % (row['page_id'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/gene/detail/%s/comment/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['page_id'], row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Comments')
        table.set_subtitle('List of gene comments')
        table.create_button('create', '/gene/detail/%s/comment/create' % (self.params['gene_id']), 'zmdi-plus')
...
class Create(actions.crud.CreateAction):
    ...
    def process_form_data(self, data):
        data['entity'] = self.params['gene_id']
        data['page'] = 'gene'
...
class Delete(actions.crud.DeleteAction):
    def GET(self):
        ...
        return HttpResponseRedirect('/gene/detail/%s/comment/list' % self.params['gene_id'])
