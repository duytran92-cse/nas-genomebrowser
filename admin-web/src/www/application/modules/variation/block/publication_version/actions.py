...
class List(actions.crud.ListAction):
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        return components.VariationBlockPublicationVersionStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
...
class Update(actions.crud.UpdateAction):
    ...
    def create_form(self):
        ...
        form.set_title('Variation block publication versions')
    ...
    def load_form(self, form):
        result = components.VariationBlockPublicationVersionStore(self.get_container()).get(self.params['pub_id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.VariationBlockPublicationVersionStore(self.get_container()).update(data, self.params['pub_id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.VariationBlockPublicationVersionStore(self.get_container()).delete(self.params['pub_id'])
...
class Active(actions.crud.DeleteAction):
    def GET(self):
        result = components.VariationBlockPublicationVersionStore(self.get_container()).active(self.params['text_id'])
        return HttpResponseRedirect('/variation/detail/%s/block/detail/%s/pub_version/list' % (self.params['variation_id'], self.params['variation_block_id']))
