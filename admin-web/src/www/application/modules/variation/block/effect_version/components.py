...
class VariationBlockEffectVersionStore(object):
    ...
    def list(self, params={}, sortkey='id', sortdir='desc', page_number=1):
        ...
        data = self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_eff_version/list', GET=params)
        return data['data']
    def get(self, eff_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_eff_version/get', GET={'eff_id': eff_id})
    def update(self, data, eff_id):
        data['eff_id'] = eff_id
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_eff_version/update', POST=data)
    def delete(self, eff_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_eff_version/delete', POST={'eff_id': eff_id})
    def active(self, text_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_eff_version/active', POST={'id': text_id})
...
class FullPageContext(page_contexts.FullPageContext):
    def __init__(self, params, container):
        ...
        if 'variation_id' in params and  'variation_block_id' in params:
            self.subsubmenu.create_menu_group('list', 'Effect Versions', '/variation/detail/%s/block/detail/%s/eff_version/list' % (str(params['variation_id']), str(params['variation_block_id'])), 'zmdi-border-all')
