...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/variation/detail/%s/block/detail/%s/infobox_version/update/%s">Edit</a>' % (row['variation'], row['variation_block'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/variation/detail/%s/block/detail/%s/infobox_version/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['variation'], row['variation_block'], row['id'])
            html += '    <a class="btn btn-xs btn-info" href="/variation/detail/%s/block/detail/%s/infobox_version/active/%s">Active</a>'  % (row['variation'], row['variation_block'], row['id'])
            html += '</div>'
            return html
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        return components.VariationBlockInfoboxVersionStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
...
class Update(actions.crud.UpdateAction):
    ...
    def create_form(self):
        ...
        form.set_title('Variation block infobox versions')
    ...
    def load_form(self, form):
        result = components.VariationBlockInfoboxVersionStore(self.get_container()).get(self.params['infobox_id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.VariationBlockInfoboxVersionStore(self.get_container()).update(data, self.params['infobox_id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.VariationBlockInfoboxVersionStore(self.get_container()).delete(self.params['infobox_id'])
        return HttpResponseRedirect('/variation/detail/%s/block/detail/%s/infobox_version/list' % (self.params['variation_id'], self.params['variation_block_id']))

class Active(actions.crud.DeleteAction):
    def GET(self):
        result = components.VariationBlockInfoboxVersionStore(self.get_container()).active(self.params['text_id'])
        return HttpResponseRedirect('/variation/detail/%s/block/detail/%s/infobox_version/list' % (self.params['variation_id'], self.params['variation_block_id']))
