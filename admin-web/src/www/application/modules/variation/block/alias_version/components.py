...
class VariationBlockAliasVersionStore(object):
    ...
    def list(self, params={}, sortkey='id', sortdir='desc', page_number=1):
        ...
        data = self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_alias_version/list', GET=params)
        return data['data']
    def get(self, alias_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_alias_version/get', GET={'alias_id': alias_id})
    def update(self, data, alias_id):
        data['alias_id'] = alias_id
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_alias_version/update', POST=data)
    def delete(self, alias_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_alias_version/delete', POST={'alias_id': alias_id})
    def active(self, text_id):
        return self.container.call_api(settings.GENOME_BROWSER_API_URL + '/variation_block_alias_version/active', POST={'id': text_id})
...
class FullPageContext(page_contexts.FullPageContext):
    def __init__(self, params, container):
        ...
        if 'variation_id' in params and  'variation_block_id' in params:
            self.subsubmenu.create_menu_group('list', 'Alias Versions', '/variation/detail/%s/block/detail/%s/alias_version/list' % (str(params['variation_id']), str(params['variation_block_id'])), 'zmdi-border-all')
