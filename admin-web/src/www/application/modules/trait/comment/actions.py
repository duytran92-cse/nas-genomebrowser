...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/trait/detail/%s/comment/update/%s">Edit</a>' % (row['page_id'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/trait/detail/%s/comment/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['page_id'], row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Comments')
        table.set_subtitle('List of trait comments')
        table.create_button('create', '/trait/detail/%s/comment/create' % (self.params['trait_id']), 'zmdi-plus')
...
class Create(actions.crud.CreateAction):
    ...
    def process_form_data(self, data):
        data['entity'] = self.params['trait_id']
        data['page'] = 'trait'
...
class Delete(actions.crud.DeleteAction):
    def GET(self):
        ...
        return HttpResponseRedirect('/trait/detail/%s/comment/list' % self.params['trait_id'])
