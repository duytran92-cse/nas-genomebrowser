...
class List(actions.crud.ListAction):
    def create_page_context(self):
        return trait_components.FullPageContext(self.params, self.container)
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/trait/detail/%s/block/update/%s">Edit</a>' % (row['trait'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/trait/detail/%s/block/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['trait'], row['id'])
            if row['kind'] == 'general_text':
                html += '    <a class="btn btn-xs btn-info" href="/trait/detail/%s/block/detail/%s/version/list">Versions</a>' % (row['trait'], row['id'])
            elif row['kind'] == 'general_publications':
                html += '    <a class="btn btn-xs btn-info" href="/trait/detail/%s/block/detail/%s/pub_version/list">Versions</a>' % (row['trait'], row['id'])
            elif row['kind'] == 'general_alias':
                html += '    <a class="btn btn-xs btn-info" href="/trait/detail/%s/block/detail/%s/alias_version/list">Versions</a>' % (row['trait'], row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Blocks')
        table.set_subtitle('List of trait blocks')
        table.create_button('create', '/trait/detail/%s/block/create' % (self.params['trait_id']), 'zmdi-plus')
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        table_form_data['trait_id'] = self.params['trait_id']
        return components.TraitBlockStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
...
class Create(actions.crud.CreateAction):
    ...
    def create_form(self):
        ...
        form.renderer.add_section('Trait block')
    ...
    def process_form_data(self, data):
        data['trait_id'] = self.params['trait_id']
        res = components.TraitBlockStore(self.get_container()).create(data)
        self.params['id'] = res['data']['pk']
        return res
    def handle_on_success(self, messages):
        return HttpResponseRedirect('/trait/detail/%s/block/update/%s' % (self.params['trait_id'],self.params['id']))
...
class Update(actions.crud.UpdateAction):
    def create_page_context(self):
        return trait_components.FullPageContext(self.params, self.container)
    def create_form(self):
        trait_block = components.TraitBlockStore(self.get_container()).get(self.params['id'])
        ...
        return form
    def load_form(self, form):
        result = components.TraitBlockStore(self.get_container()).get(self.params['id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.TraitBlockStore(self.get_container()).update(data, self.params['id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.TraitBlockStore(self.get_container()).delete(self.params['id'])
        return HttpResponseRedirect('/trait/detail/%s/block/list' % self.params['trait_id'])
