...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/trait/detail/%s/block/detail/%s/alias_version/update/%s">Edit</a>' % (row['trait'], row['trait_block'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/trait/detail/%s/block/detail/%s/alias_version/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['trait'], row['trait_block'], row['id'])
            html += '    <a class="btn btn-xs btn-info" href="/trait/detail/%s/block/detail/%s/alias_version/active/%s">Active</a>'  % (row['trait'], row['trait_block'], row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Block Versions')
        table.set_subtitle('List of trait block versions')
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        table_form_data['trait_block_id'] = self.params['trait_block_id']
        table_form_data['trait_id'] = self.params['trait_id']
        return components.TraitBlockAliasVersionStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
...
class Update(actions.crud.UpdateAction):
    ...
    def create_form(self):
        ...
        form.set_title('Trait block alias versions')
        ...
        form.renderer.add_section('Trait block versions')
        ...
        form.renderer.add_section('Trait - Alias')
    ...
    def load_form(self, form):
        result = components.TraitBlockAliasVersionStore(self.get_container()).get(self.params['alias_id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.TraitBlockAliasVersionStore(self.get_container()).update(data, self.params['alias_id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.TraitBlockAliasVersionStore(self.get_container()).delete(self.params['alias_id'])
        return HttpResponseRedirect('/trait/detail/%s/block/detail/%s/alias_version/list' % (self.params['trait_id'], self.params['trait_block_id']))

class Active(actions.crud.DeleteAction):
    def GET(self):
        result = components.TraitBlockAliasVersionStore(self.get_container()).active(self.params['text_id'])
        return HttpResponseRedirect('/trait/detail/%s/block/detail/%s/alias_version/list' % (self.params['trait_id'], self.params['trait_block_id']))

class ActiveSum(actions.crud.DeleteAction):
    def GET(self):
        result = components.TraitBlockAliasVersionStore(self.get_container()).active(self.params['text_id'])
        return HttpResponseRedirect('/trait/summary/%s' % (self.params['trait_id']))
