...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/disease/detail/%s/block/update/%s">Edit</a>' % (row['disease'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/disease/detail/%s/block/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['disease'], row['id'])
            if row['kind'] == 'general_text':
                html += '    <a class="btn btn-xs btn-info" href="/disease/detail/%s/block/detail/%s/version/list">Versions</a>' % (row['disease'], row['id'])
            elif row['kind'] == 'general_publications':
                html += '    <a class="btn btn-xs btn-info" href="/disease/detail/%s/block/detail/%s/pub_version/list">Versions</a>' % (row['disease'], row['id'])
            elif row['kind'] == 'general_alias':
                html += '    <a class="btn btn-xs btn-info" href="/disease/detail/%s/block/detail/%s/alias_version/list">Versions</a>' % (row['disease'], row['id'])
            html += '</div>'
            return html
    def create_table(self):
        table = widgets.table.DataTable()
        table.set_title('Blocks')
        table.set_subtitle('List of disease blocks')
        table.create_button('create', '/disease/detail/%s/block/create' % (self.params['disease_id']), 'zmdi-plus')
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        data = components.DiseaseBlockStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
        return data
...
class Create(actions.crud.CreateAction):
    ...
    def create_form(self):
        ...
        form.renderer.add_section('Disease block')
    ...
    def process_form_data(self, data):
        data['disease_id'] = self.params['disease_id']
        res = components.DiseaseBlockStore(self.get_container()).create(data)
        self.params['id'] = res['data']['pk']
        return res
    def handle_on_success(self, messages):
        return HttpResponseRedirect('/disease/detail/%s/block/update/%s' % (self.params['disease_id'],self.params['id']))
...
class Update(actions.crud.UpdateAction):
    def create_page_context(self):
        return disease_components.FullPageContext(self.params, self.container)
    def create_form(self):
        disease_block = components.DiseaseBlockStore(self.get_container()).get(self.params['id'])
        kind = disease_block['data']['record']['kind']
        ...
        return form
    def load_form(self, form):
        result = components.DiseaseBlockStore(self.get_container()).get(self.params['id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.DiseaseBlockStore(self.get_container()).update(data, self.params['id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.DiseaseBlockStore(self.get_container()).delete(self.params['id'])
        return HttpResponseRedirect('/disease/detail/%s/block/list' % self.params['disease_id'])
