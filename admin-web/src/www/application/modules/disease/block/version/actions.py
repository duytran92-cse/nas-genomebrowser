...
class List(actions.crud.ListAction):
    ...
    class TableRenderer(renderers.widgets.table.DataTableRenderer):
        def render_cell_actions(self, table, row):
            ...
            html += '    <a class="btn btn-xs btn-primary" href="/disease/detail/%s/block/detail/%s/version/update/%s">Edit</a>' % (row['disease'], row['disease_block'], row['id'])
            html += '    <a class="btn btn-xs btn-danger" href="/disease/detail/%s/block/detail/%s/version/delete/%s" onclick="return confirm(\'Are you really want to delete this?\')">Delete</a>'  % (row['disease'], row['disease_block'], row['id'])
            html += '    <a class="btn btn-xs btn-info" href="/disease/detail/%s/block/detail/%s/version/active/%s">Active</a>'  % (row['disease'], row['disease_block'], row['id'])
            html += '</div>'
            return html
    ...
    def load_table_data(self, table_form_data, sortkey, sortdir, page_number):
        ...
        return components.DiseaseBlockVersionStore(self.get_container()).list(table_form_data, sortkey, sortdir, page_number)
...
class Update(actions.crud.UpdateAction):
    ...
    def create_form(self):
        ...
        form.set_title('Disease block versions')
    ...
    def load_form(self, form):
        result = components.DiseaseBlockVersionStore(self.get_container()).get(self.params['text_id'])
        if result['status'] == 'ok':
    ...
    def process_form_data(self, data):
        return components.DiseaseBlockVersionStore(self.get_container()).update(data, self.params['text_id'])


class Delete(actions.crud.DeleteAction):
    def GET(self):
        result = components.DiseaseBlockVersionStore(self.get_container()).delete(self.params['text_id'])
        return HttpResponseRedirect('/disease/detail/%s/block/detail/%s/eff_version/list' % (self.params['disease_id'], self.params['disease_block_id']))

class Active(actions.crud.DeleteAction):
    def GET(self):
        result = components.DiseaseBlockVersionStore(self.get_container()).active(self.params['text_id'])
        return HttpResponseRedirect('/disease/detail/%s/block/detail/%s/version/list' % (self.params['disease_id'], self.params['disease_block_id']))
